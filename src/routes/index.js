...
export default function Routes() {
  return (
    <Tab.Navigator screenOptions={{tabBarShowLabel:false, tabBarActiveTintColor:'red', tabBarInactiveTintColor:"#474a51"}}>
        <Tab.Screen name='Inicio' component={Inicio} options={{headerShown:false, tabBarIcon:({color,size}) => {
            return <FontAwesome name='home' color={color} size={size}></FontAwesome>
        }}}>
            
        </Tab.Screen>
        <Tab.Screen name='Cursos' component={Cursos} options={{tabBarIcon:({color,size}) => {
            return <FontAwesome name='book' color={color} size={size}></FontAwesome>
        }}}></Tab.Screen>
        <Tab.Screen name='Contatos' component={Contatos} options={{tabBarIcon:({color,size}) => {
